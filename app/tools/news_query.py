...
NEWS_TOOL_DEFINITION = {
    "name": "news_query",
    "description": "新闻查询工具。根据查询类型获取新闻数据。",
    "parameters": {
        "type": "object",
        "properties": {
            "query_type": {
                "type": "string",
                "description": "查询类型：search(搜索), hot(热门), recent(最近), author(作者), stats(统计), today(今天), week(本周), month(本月)",
                "enum": ["search", "hot", "recent", "author", "stats", "today", "week", "month"]
            },
            "keyword": {
                "type": "string",
                "description": "搜索关键词（query_type为search时使用）"
            },
            "author": {
                "type": "string",
                "description": "作者名称（query_type为author时使用）"
            },
            "limit": {
                "type": "integer",
